...
public:
    SimpleParameter(): Node("simple_parameter"){
        declare_parameter<int>("int_param", 20);
        declare_parameter<std::string>("string_param", "Hello!");
    ...
    rcl_interfaces::msg::SetParametersResult paramChangeCallback(const std::vector<rclcpp::Parameter> &parameters){
        ...
        for(auto &param : parameters){
            if(param.get_name()== "int_param" && param.get_type()==rclcpp::ParameterType::PARAMETER_INTEGER){
                RCLCPP_INFO_STREAM(get_logger(), "Param int_param has changed and new value is: " << param.as_int());
                result.successful= true;
            }

            if(param.get_name()== "string_param" && param.get_type()==rclcpp::ParameterType::PARAMETER_STRING){
                RCLCPP_INFO_STREAM(get_logger(), "Param int_param has changed and new value is: " << param.as_string());
                result.successful= true;
            }
        }
        return result;
